...
angular.module('bioshockLogin')
  .controller('bioshockLoginController', function ($scope, $location, $http, $timeout, khAccountAccount, khAccountOauth) {
    'use strict';

    //need these for forms to work properly
    $scope.credentials = {};
    ...
    $scope.loggingIn = function (form) {
      ...
      $scope.accountFunc = khAccountAccount.login;
    ...
    $scope.signingUp = function (form) {
      ...
      if (form.pw.$viewValue === form.pwconf.$viewValue) {
        ...
        $scope.accountFunc = khAccountAccount.createAccount;
    ...
    $scope.loginFacebook = function () {
      //get the oauth token
      khAccountOauth.getToken(khAccountOauth.grantors.Facebook).then(
        function (token) {
          ...
          return $http.post('/api/oauth/deflists', {token: token}).then(
            function (res) {
              if (res.data.deflists) {
                ...
                $scope.accountFunc = khAccountOauth.callURL;
              } else {
